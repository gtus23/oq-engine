...
sigma_mu_csv = os.path.join(os.path.dirname(__file__), 'kotha_2020.csv')
...
class KothaEtAl2020(GMPE):
    ...
    #: The large-magnitude statistical standard deviation values are taken from
    #: data supplied by Kotha et al. (2020)
    COEFFS_SIGMA_MU = CoeffsTable(
        sa_damping=5, table=open(sigma_mu_csv).read())
    ...
    def get_sigma_mu_adjustment(self, C, imt, rup):
        ...
        C_SIG_MU = self.COEFFS_SIGMA_MU[imt]
...
class KothaEtAl2020Slope(KothaEtAl2020):
    ...
    # Defines the c3 distribution (expected and variance [tau]) for each of
...
class KothaEtAl2020ESHM20(KothaEtAl2020):
    ...
    REQUIRES_SITES_PARAMETERS = {"region", "vs30", "vs30measured"}

    HETERO_PHI0 = KothaEtAl2020Slope.HETERO_PHI0
    C3_REGIONS = KothaEtAl2020Slope.C3_REGIONS
